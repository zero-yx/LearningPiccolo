...
namespace Piccolo
{
    ...
    void MainCameraPass::setupRenderPass()
    {
        ...
        gbuffer_metallic_roughness_shadingmodeid_attachment_description.stencilLoadOp =
    ...
    void MainCameraPass::setupDescriptorSetLayout()
    {
...

        {
            ...
            if (m_rhi->createDescriptorSetLayout(&mesh_mesh_layout_create_info, m_descriptor_infos[_per_mesh].layout) !=
                RHI_SUCCESS)
            {
...

        {
            ...
            if (RHI_SUCCESS != m_rhi->createDescriptorSetLayout(&mesh_global_layout_create_info,
...

        {
            ...
            if (m_rhi->createDescriptorSetLayout(&mesh_material_layout_create_info,
                                                 m_descriptor_infos[_mesh_per_material].layout) != RHI_SUCCESS)
            {
...

        {
            ...
            if (RHI_SUCCESS !=
                m_rhi->createDescriptorSetLayout(&skybox_layout_create_info, m_descriptor_infos[_skybox].layout))
            {
...

        {
            ...
            if (RHI_SUCCESS !=
                m_rhi->createDescriptorSetLayout(&axis_layout_create_info, m_descriptor_infos[_axis].layout))
            {
    ...
    void MainCameraPass::setupPipelines()
    {
        ...
        // mesh gbuffer
        {
            ...
            if (RHI_SUCCESS !=
                m_rhi->createGraphicsPipelines(
                    RHI_NULL_HANDLE, 1, &pipelineInfo, m_render_pipelines[_render_pipeline_type_mesh_gbuffer].pipeline))
            {
        ...
        // draw axis
        {
            ...
            if (m_rhi->createPipelineLayout(&pipeline_layout_create_info,
                                            m_render_pipelines[_render_pipeline_type_axis].layout) != RHI_SUCCESS)
            {
            ...
            if (RHI_SUCCESS !=
                m_rhi->createGraphicsPipelines(
                    RHI_NULL_HANDLE, 1, &pipelineInfo, m_render_pipelines[_render_pipeline_type_axis].pipeline))
            {
    ...
    void MainCameraPass::setupSkyboxDescriptorSet()
    {
        ...
        if (RHI_SUCCESS != m_rhi->allocateDescriptorSets(&skybox_descriptor_set_alloc_info,
    ...
    void MainCameraPass::setupAxisDescriptorSet()
    {
        ...
        if (RHI_SUCCESS !=
            m_rhi->allocateDescriptorSets(&axis_descriptor_set_alloc_info, m_descriptor_infos[_axis].descriptor_set))
        {
    ...
    void MainCameraPass::drawAxis()
    {
        ...
        m_rhi->cmdDrawIndexedPFN(
            m_rhi->getCurrentCommandBuffer(), m_visiable_nodes.p_axis_node->ref_mesh->mesh_index_count, 1, 0, 0, 0);
